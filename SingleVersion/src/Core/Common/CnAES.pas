...
function PKCS5_DePadding(Bytes: TBytes): AnsiString;
...
begin
  ...
  try
    for I := 0 to Len - 1 do
    begin
      lBuilder.Append(IntToHex(Ord(Value[I]), 2));
      //Result := Result + IntToHex(Ord(Value[I]), 2);
    end;
    Result := AnsiString(lBuilder.ToString);
  finally
    lBuilder.Free;
  end;
end;
...
begin
  ...
  Result := AnsiString(Encoding.GetString(tmpBytes, Size, Length(tmpBytes) - Size));
end;
