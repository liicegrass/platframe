...
function SOLoadFrom(const FileName: string): ISuperObject; overload;
...
function SOLoadFrom(const FileName: string): ISuperObject;
var
  s: TStringStream;
begin
...
var
  k1, k2: SOString;
begin
  // Modify by lilb 修改为不区分大小写，全部转换为小写后计算
  k1 := LowerCase(k);
  k2 := LowerCase(h.FName);
  Result := StrComp(PSOChar(k1), PSOChar(k2));
...
begin
  // Modify by lilb 修改为不区分大小写，全部转换为小写后计算
  k1 := LowerCase(node1.FName);
  k2 := LowerCase(node2.FName);
  Result := StrComp(PSOChar(k1), PSOChar(k2));
...
{$Q-}
  // Modify by lilb 修改为不区分大小写，全部转换为小写后计算
  lk := LowerCase(k);
  for i := 1 to Length(lk) do
    h := h*129 + ord(lk[i]) + $9e370001;
